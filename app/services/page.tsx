    ...
    <main className="w-full overflow-x-hidden bg-[#fafbfc]">
      <section className="pt-28 sm:pt-36 md:pt-44 pb-16 sm:pb-24 max-w-[1440px] mx-auto px-4 sm:px-6 md:px-12 lg:px-20">
        <div className="max-w-5xl">
          <span className="font-mono text-[10px] sm:text-xs font-semibold uppercase tracking-[0.22em] text-[#2e4dff] block mb-5">
            Systems & capabilities
          </span>
          <h1 className="font-display font-extrabold text-4xl sm:text-6xl md:text-7xl lg:text-8xl tracking-tight text-[#171a21] leading-[0.95]">
            The problem determines the system. The system determines the technology.
          </h1>
          <p className="mt-6 sm:mt-8 max-w-3xl text-sm sm:text-lg md:text-xl text-[#636873] leading-relaxed">
      ...
      <section className="border-y border-[#171a21]/[0.08] bg-white">
        <div className="max-w-[1440px] mx-auto px-4 sm:px-6 md:px-12 lg:px-20 py-14 sm:py-20">
          <AnimatedSection className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 mb-10">
            <div className="lg:col-span-5">
              <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-[#2e4dff]">A good fit when</span>
              <h2 className="mt-4 font-display text-3xl sm:text-5xl font-extrabold text-[#171a21] tracking-tight">The operation has become the constraint.</h2>
            </div>
            <p className="lg:col-span-5 lg:col-start-8 lg:pt-8 text-sm sm:text-lg text-[#636873] leading-relaxed">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-[#171a21]/[0.08] border border-[#171a21]/[0.08]">
            {CONDITIONS.map((condition, index) => (
              <div key={condition} className="bg-white p-5 sm:p-7 flex gap-4">
                <span className="font-mono text-[10px] text-[#2e4dff]">0{index + 1}</span>
                <p className="text-sm text-[#171a21] leading-relaxed">{condition}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-16 sm:py-24 md:py-32 max-w-[1440px] mx-auto px-4 sm:px-6 md:px-12 lg:px-20">
        <AnimatedSection className="max-w-4xl mb-10 sm:mb-16">
          <span className="font-mono text-[10px] sm:text-xs font-semibold uppercase tracking-[0.22em] text-[#2e4dff] block mb-4">Solution layers</span>
          <h2 className="font-display font-extrabold text-3xl sm:text-5xl md:text-6xl tracking-tight text-[#171a21] leading-[1.02]">
            Capabilities combined around one operational outcome.
          </h2>
        </AnimatedSection>

        <div className="border-t border-[#171a21]/[0.08]">
          {LAYERS.map((layer, index) => (
            <AnimatedSection key={layer.number} delay={((index % 4) + 1) as 1 | 2 | 3 | 4}>
              <article className="grid grid-cols-[44px_1fr] lg:grid-cols-[70px_1.1fr_1.5fr_1fr] gap-4 lg:gap-8 py-7 sm:py-10 border-b border-[#171a21]/[0.08] items-start">
                <span className="font-mono text-xs text-[#2e4dff]">{layer.number}</span>
                <div>
                  <h3 className="font-display text-xl sm:text-2xl font-bold text-[#171a21]">{layer.title}</h3>
                  <span className="block mt-2 font-mono text-[9px] uppercase tracking-wider text-[#87909f]">{layer.when}</span>
                </div>
                <p className="col-start-2 lg:col-start-auto text-xs sm:text-sm text-[#636873] leading-relaxed">{layer.meaning}</p>
                <p className="col-start-2 lg:col-start-auto font-mono text-[9px] sm:text-[10px] uppercase tracking-wider text-[#87909f]">{layer.output}</p>
              </article>
            </AnimatedSection>
          ))}
        </div>
      ...
      <section className="py-16 sm:py-24 px-4 sm:px-6 md:px-12 lg:px-20 border-t border-[#171a21]/[0.08]">
        <AnimatedSection className="max-w-[1440px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16">
          <div className="lg:col-span-7">
            <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-[#2e4dff]">The first engagement</span>
            <h2 className="mt-4 font-display text-3xl sm:text-5xl font-extrabold text-[#171a21] tracking-tight">Start by mapping the operational gap.</h2>
            <p className="mt-5 max-w-2xl text-sm sm:text-lg text-[#636873] leading-relaxed">
          ...
          <div className="lg:col-span-4 lg:col-start-9 lg:self-end">
            <Link href="/contact" className="w-full inline-flex items-center justify-center h-13 sm:h-14 px-6 bg-[#2e4dff] text-white text-xs font-bold uppercase tracking-[0.15em] rounded-[4px] hover:bg-[#1534dc] transition-colors">
